...
# Set default portlib
set(PORTLIB_PATH ${DEVKITPRO}/portlibs CACHE STRING "Path to portlibs (if used)")
set(PORT_LIBRARY_DIR_GCN ${PORTLIB_PATH}/gamecube)
set(PORT_LIBRARY_DIR_WII ${PORTLIB_PATH}/wii)
set(PORT_LIBRARY_DIR_SHARED ${PORTLIB_PATH}/ppc)
include_directories(${PORT_LIBRARY_DIR_SHARED}/include ${PORT_LIBRARY_DIR_GCN}/include ${PORT_LIBRARY_DIR_WII}/include)
...
function(add_port_libraries target)
    foreach(libname ${ARGN})
        ...
        if(EXISTS "${PORT_LIBRARY_DIR_SHARED}/lib/lib${libname}.a")
            if(GCN)
                target_link_libraries(${target}_gcn "${PORT_LIBRARY_DIR_SHARED}/lib/lib${libname}.a")
            endif()
            if(WII)
                target_link_libraries(${target}_wii "${PORT_LIBRARY_DIR_SHARED}/lib/lib${libname}.a")
            endif()
        else()
            if(GCN)
                target_link_libraries(${target}_gcn "${PORT_LIBRARY_DIR_GCN}/lib/lib${libname}.a")
            endif()
            if(WII)
                target_link_libraries(${target}_wii "${PORT_LIBRARY_DIR_WII}/lib/lib${libname}.a")
            endif()
        endif()
    endforeach()
endfunction()
